...
keySetForFeeCalculation.push({
  priv,
  ecdsaPub: secp256k1.getPublicKey(priv, true),
  schnorrPub: secp256k1.getPublicKey(priv, false),
});

const testWallets = [
  {
    privateKey:
      "ad1195070a559967782fb6eaa622a2baeaed9d9d254880059f9fbf781cf7852c",
    ecdsaPub:
      "0235bbcc0b6898fc63d6e856c10b67490b153f8866a88b7e59b2229fb2dc9cf102",
    schnorrPub:
      "0435bbcc0b6898fc63d6e856c10b67490b153f8866a88b7e59b2229fb2dc9cf102369bdef88e0c63b560a7d5295347e6dc6cd9d2158a8edc906ba09ac1019db0f8",
  },
  {
    privateKey:
      "b3fd3a7216621aa796270da8149298a6f1cbf2eba4a4fc3cc21725f289d2551d",
    ecdsaPub:
      "0235bbcc0b6898fc63d6e856c10b67490b153f8866a88b7e59b2229fb2dc9cf102",
    schnorrPub:
      "0435bbcc0b6898fc63d6e856c10b67490b153f8866a88b7e59b2229fb2dc9cf102369bdef88e0c63b560a7d5295347e6dc6cd9d2158a8edc906ba09ac1019db0f8",
...
export const sbtcWallets = [
  {
    ...
    parent_desc:
      "tr([760ce8cf/86'/1'/0']tpubDDQtKohNhMryjsYgQu8hsZ1BMXJWb1h4xGDZvsQV5ZK9E5QDNgp3w1h9N2XTyz6GVDmMcbAw5YU67mcGousktHxjVTx6RmqXX6GfJJrkqqh/0/*)#kqt0kevz",
    scriptPubKey:
      "51204faa61bcd4f553d1ca945d6f74b18f60705d85191f61d76d34158b0e7798b710",
    witness_program:
      "4faa61bcd4f553d1ca945d6f74b18f60705d85191f61d76d34158b0e7798b710",
  },
  {
    sbtcAddress:
    ...
    parent_desc:
      "tr([7e0bf729/86'/1'/0']tpubDCzcBRDqD1G23fAdF79sTfdECnfRprb5uGKb9vKBxrH4uZbC46ZJmxtSdYHwEJykzuzZV3KUGtFSRaoNAJuZpRSCiKoC1FUxkmRjPjDrbSA/0/*)#a8uhq8yj",
    scriptPubKey:
      "5120deec29b36f0829c98748cf1c3271497ece61e9aa5148242b23378f39d9fb854f",
    witness_program:
      "deec29b36f0829c98748cf1c3271497ece61e9aa5148242b23378f39d9fb854f",
  },
];
/**
...
export function addInputs(
  ...
  for (const utxo of utxos) {
    ...
    const script = btc.RawTx.decode(hex.decode(hexy));
    if (amt < bar && utxo.status.confirmed) {
      const txFromUtxo = btc.Transaction.fromRaw(hex.decode(hexy), {
      ...
      if (spendScr.type === "sh") {
        ...
        for (let i = 0; i < 10; i++) {
          try {
            if (i === 0) {
              ...
              p2shObj = btc.p2sh(
                btc.p2wsh(btc.p2wpkh(hex.decode(paymentPublicKey))),
                net
              );
            } else if (i === 2) {
              p2shObj = btc.p2sh(
                btc.p2wsh(btc.p2pkh(hex.decode(paymentPublicKey)), net)
              );
            } else if (i === 3) {
              p2shObj = btc.p2sh(
                btc.p2ms(1, [hex.decode(paymentPublicKey)]),
                net
              );
            } else if (i === 4) {
              p2shObj = btc.p2sh(btc.p2pkh(hex.decode(paymentPublicKey)), net);
              ...
              p2shObj = btc.p2sh(
                btc.p2sh(btc.p2pkh(hex.decode(paymentPublicKey)), net)
              );
            } else if (i === 6) {
              p2shObj = btc.p2sh(
                btc.p2sh(btc.p2wpkh(hex.decode(paymentPublicKey)), net)
              );
            }
            if (i < 3) {
        ...
        const spendAddr = getAddressFromOutScript(
          network,
...
export function getPegWalletAddressFromPublicKey(
  ...
  if (!sbtcWalletPublicKey) throw new Error("no sbtc Wallet Public Key");
