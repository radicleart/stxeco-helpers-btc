...
import { getAddressFromOutScript, getNet } from "./wallet_utils";
...
import { PayloadType } from "./sbtc_types";
import {
  PubKeyEncoding,
 ...
 * @returns
 */
export async function parsePayloadFromTransaction(
  network: string,
  txHex: string
): Promise<PayloadType> {
...
export function parsePayloadFromOutput(
  ...
  const vout1Address = getAddressFromOutScript(
    network,
    tx.getOutput(1).script!
  );
  let payload = parseRawPayload(
    network,
    hex.encode(tx.getOutput(0).script!),
    vout1Address,
    "vrs"
  );
  return payload;
