...
export function getAddressFromHashBytes(
  netowrk: string,
  hashBytes: string,
  version: string
) {
  ...
  try {
    ...
    btcAddr = err.message;
    console.error(
      "getAddressFromHashBytes: version:hashBytes: " + version + ":" + hashBytes
    );
  }
  return btcAddr;
}

export function getHashBytesFromAddress(
  netowrk: string,
  address: string
): { version: string; hashBytes: string } | undefined {
  const net = netowrk === "testnet" ? btc.TEST_NETWORK : btc.NETWORK;
  let outScript: any;
  try {
    const addr: any = btc.Address(net);
