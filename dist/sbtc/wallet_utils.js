...
exports.sbtcWallets = exports.REGTEST_NETWORK = void 0;
exports.getNet = getNet;
exports.getTestAddresses = getTestAddresses;
 ...
 * test wallets.
 */
function getTestAddresses(network) {
    const net = getNet(network);
    return {
        fromBtcAddress: btc.getAddress("tr", base_1.hex.decode(testWallets[0].privateKey), net),
        sbtcWalletAddress: exports.sbtcWallets[0].sbtcAddress,
        //reveal: btc.getAddress('tr', hex.decode(testWallets[0].privateKey), net) as string,
        revealPub: base_1.hex.encode(secp256k1_1.schnorr.getPublicKey(testWallets[0].privateKey)),
        //revealPrv: testWallets[0].privateKey,
        //reclaim: btc.getAddress('tr', hex.decode(testWallets[1].privateKey), net) as string,
        reclaimPub: base_1.hex.encode(secp256k1_1.schnorr.getPublicKey(testWallets[1].privateKey)),
        //reclaimPrv: testWallets[1].privateKey,
        stacksAddress: network === "testnet"
            ? "ST1RBP62PR532FWVP7JRGC9SVFKKHD1JYK23KYNN0"
            : "unsupported",
    };
}
// Address from a 33 byte public key (returns the pub key if schnorr pub key passed in)
function addressFromPubkey(network, pubkey) {
    const net = getNet(network);
    try {
        ...
        return base_1.hex.encode(pubkey);
    }
}
 ...
 */
function addInputs(network, amount, txFee, transaction, feeCalc, utxos, paymentPublicKey) {
    const net = getNet(network);
    const bar = txFee + amount;
    let amt = 0;
    for (const utxo of utxos) {
        const hexy = utxo.tx.hex ? utxo.tx.hex : utxo.tx;
        const script = btc.RawTx.decode(base_1.hex.decode(hexy));
        if (amt < bar && utxo.status.confirmed) {
            const txFromUtxo = btc.Transaction.fromRaw(base_1.hex.decode(hexy), {
            ...
            if (!outputToSpend || !outputToSpend.script)
                throw new Error("no script passed ?");
            ...
            if (spendScr.type === "sh") {
                ...
                for (let i = 0; i < 10; i++) {
                    try {
                        if (i === 0) {
                            p2shObj = btc.p2sh(btc.p2wpkh(base_1.hex.decode(paymentPublicKey)), net);
                        }
                        else if (i === 1) {
                            p2shObj = btc.p2sh(btc.p2wsh(btc.p2wpkh(base_1.hex.decode(paymentPublicKey))), net);
                        }
                        else if (i === 2) {
                            p2shObj = btc.p2sh(btc.p2wsh(btc.p2pkh(base_1.hex.decode(paymentPublicKey)), net));
                        }
                        else if (i === 3) {
                            p2shObj = btc.p2sh(btc.p2ms(1, [base_1.hex.decode(paymentPublicKey)]), net);
                        }
                        else if (i === 4) {
                            p2shObj = btc.p2sh(btc.p2pkh(base_1.hex.decode(paymentPublicKey)), net);
                        }
                        else if (i === 5) {
                            p2shObj = btc.p2sh(btc.p2sh(btc.p2pkh(base_1.hex.decode(paymentPublicKey)), net));
                        }
                        else if (i === 6) {
                            p2shObj = btc.p2sh(btc.p2sh(btc.p2wpkh(base_1.hex.decode(paymentPublicKey)), net));
                        }
                        if (i < 3) {
                ...
                const nextI = Object.assign(Object.assign({ txid: base_1.hex.decode(utxo.txid), index: utxo.vout }, outputToSpend), { witnessUtxo: {
                ...
                if (feeCalc) {
                    witnessUtxo = {
                        amount: BigInt(utxo.value),
                        script: btc.p2wpkh(secp256k1_1.secp256k1.getPublicKey(privKey, true)).script,
                    };
                }
                const nextI = {
...
function redeemScriptAddInput(utxo, p2shObj, hexy) {
    return {
        txid: base_1.hex.decode(utxo.txid),
        index: utxo.vout,
        nonWitnessUtxo: hexy,
...
function redeemAndWitnessScriptAddInput(utxo, p2shObj, hexy) {
    return {
        txid: base_1.hex.decode(utxo.txid),
        index: utxo.vout,
        witnessUtxo: {
...
function isUTXOConfirmed(utxo) {
    return utxo.tx.confirmations >= 3;
}
function inputAmt(tx) {
    var _a;
    let amt = 0;
    for (let idx = 0; idx < tx.inputsLength; idx++) {
        const inp = tx.getInput(idx);
        if (inp.witnessUtxo)
            amt += Number((_a = tx.getInput(idx).witnessUtxo) === null || _a === void 0 ? void 0 : _a.amount);
        else if (inp.nonWitnessUtxo)
            amt += Number(inp.nonWitnessUtxo.outputs[inp.index].amount);
    }
    return amt;
}
/**
...
function toXOnly(pubkey) {
    return base_1.hex.encode(base_1.hex.decode(pubkey).subarray(1, 33));
}
/**
 *
 * @param network
...
function getPegWalletAddressFromPublicKey(network, sbtcWalletPublicKey) {
    if (!sbtcWalletPublicKey)
        throw new Error("no sbtc Wallet Public Key");
